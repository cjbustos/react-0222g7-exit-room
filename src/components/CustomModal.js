...
const CustomModal = ({ name, label, sizeButton }) => {
    ...
    const handleChange = (e) => {
        setShoppingCart({
            ...shoppingCart, [e.target.name] : e.target.value
        });
    };
        ...
        <div>
            ...
            <ReactModal isOpen={isOpen} onRequestClose={toggleModal} className="Modal" overlayClassName="Overlay">
                <form onSubmit={handleSubmit}>
                    ...
                    <h1>{name}</h1>
                        <select defaultValue={shoppingCart.city} name="city" disabled={active} onChange={handleChange}>
                        ...
                        <select defaultValue={shoppingCart.date} name="date" disabled={active} onChange={handleChange}>
                            <option value={shoppingCart.date} disabled>Seleccione fecha</option>
                            {dates.map((el, index) => <option key={index} value={el}>{el}</option>)}
                        </select>
                        <select defaultValue={shoppingCart.info} name="info" disabled={active} onChange={handleChange}>
